(function() {
  ...
  function storageFor(name) {
    ...
    storage.caches = Object.create(null);
    storage["food caches"] = ["cache in the oak", "cache in the meadow", "cache under the hedge"]
    storage.caches["cache in the oak"] = "A hollow above the third big branch from the bottom. Several pieces of bread and a pile of acorns."
    storage.caches["cache in the meadow"] = "Buried below the patch of nettles (south side). A dead snake."
    storage.caches["cache under the hedge"] = "Middle of the hedge at Gilles' garden. Marked with a forked twig. Two bottles of beer."
  ...
  const $storage = Symbol("storage"), $network = Symbol("network"), $caches = Symbol("caches");
  ...
  class Node {
    ...
    readCache(cacheName, callback) {
      let caches = JSON.parse(this[$storage]["caches"])
      setTimeout(() => callback(caches[cacheName]), 10)
    }
